...
export function setTab(num) {
  //myTabPosition = num;
  sessionStorage.setItem("tabpos", num);
  cdRefresh();
}
...
function Logout() {
  ...
  // sessionStorage.setItem("newTabPos", "0)
...
function getTabPos() {
  let pos = 0;
  //console.log(`My tab position is ${sessionStorage.getItem("tabpos")}`)
  if ((sessionStorage.getItem("tabpos") === null)  ||
        (sessionStorage.getItem("tabpos") === "") ||
        (sessionStorage.getItem("tabpos") === "0")) {
        //console.log("Setting TABPOS to 0")
        pos = 0;  
  } else {
    //console.log(`Settab as per setting which is ${sessionStorage.getItem("tabpos")}`)
    pos = parseInt(sessionStorage.getItem("tabpos"));
  }
...
export function CricDreamTabs() {
  ...
  const handleChange = (event, newValue) => {
    if (newValue !== value) {
      sessionStorage.setItem("tabpos", newValue);
      setValue(newValue);
    }
  };
  // value={value}
